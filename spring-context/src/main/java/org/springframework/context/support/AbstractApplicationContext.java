...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// Prepare this context for refreshing.(准备上下文已供刷新)
			/**
			 *  做容器刷新前的准备工作
			 *  1.设置容器的启动时间
			 *  2.设置活跃状态为true
			...
			*/
			...
			//创建容器 并从资源/配置路径加载BeanDefinition  容器对象(DefaultListableBeanFactory)
			...
			try {
				...
				//调用在上下文中注册为 beans 的工厂处理器 处理BeanFactoryPostProcessor
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				//注册拦截 Bean 创建的 Bean 处理器  注册BeanPostProcessor
	...
	protected void prepareRefresh() {
		...
		//如果扩展了initPropertySources()方法 并加入了 setRequestProperties()方法 那么在这里就会验证 加入的参数 是否存在
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		//初始化BeanFactory,并进行XML文件读取,并将得到的BeanFactory 记录到当前实体的属性中
		refreshBeanFactory();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		//为beanFactory增加一个磨人的propertyEditor,这个主要是对bean的属性等设置管理的一个工具类
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		// Configure the bean factory with context callbacks.
		//添加beanPostProcessor,ApplicationContextAwareProcess用来完成某些Aware对象的注入
		...
		//增加对AspectJ的支持,在java中织入分为三种方式,编译器织入,类加载器织入,运行期织入,编译器织入是指在java编译器,使用特殊的编译器,将切面织入到java类中,
		//而类加载器织入是通过特殊的类加载器,在类字节码加载到JVM时,织入切面,运行期织入则是采用cglib和jdk进行切面的织入
		//aspectj提供了两种织入方式,第一种是通过特殊编译器,在编译器,将aspectj语言编写的切面织入到java类中,第二种是类加载器织入,就是下面的load time weaving
	...
	protected void initMessageSource() {
		...
		//首先判断是否已有xml文件定义了 ID为messageSource的Bean对象
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			if (this.parent != null && this.messageSource instanceof HierarchicalMessageSource hms &&
				...
				// registered already.
				//判断父类的MessageSource是否为空,如果等于空.设置父类的messageSource
	...
	protected void initApplicationEventMulticaster() {
		//获取当前工厂 一般是DefaultListableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//判断容器中是否存在bdName为applicationEventMulticaster的bd,也就是说自定义的时间监听多路广播器
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		//如果BeanFactory之前没有注册嵌入值解析器,则注册默认的嵌入式值解析器,主要用于注解属性值的解析
		if (!beanFactory.hasEmbeddedValueResolver()) {
			beanFactory.addEmbeddedValueResolver(strVal -> getEnvironment().resolvePlaceholders(strVal));
		}

		// Initialize LoadTimeWeaverAware beans early to allow for registering their transformers early.
		//尽早初始化LoadTimeWeaverAware bean以便尽早注册他们的解析器
		...
		//冻结所有的Bean定义,说明注册的bean定义将不被修改或任何进一步的处理		beanFactory.freezeConfiguration();
